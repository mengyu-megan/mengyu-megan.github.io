...
function run(input) // input is for current play
{
    ...
    if (m == 1) guess = 2;
    if (m == 2) guess = 0;
    $('#guess').html('Next: ' + (guess == 1 ? 'Rock' : guess == 2 ? 'Paper' : 'Scissors'));

    // visualize the state transfer matrix
    var countNormalized = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
    var sum = [count[0][0] + count[1][0] + count[2][0],
        count[0][1] + count[1][1] + count[2][1],
        count[0][2] + count[1][2] + count[2][2]];
    for (var i = 0; i < 3; i++)
        for (var j = 0; j < 3; j++)
                countNormalized[i][j] = sum[j] == 0 ? 0 : count[i][j] / sum[j];
    for (var i = 0; i < 3; i++)
        for (var j = 0; j < 3; j++)
            $('#t' + i.toString() + j.toString()).html(Math.round(countNormalized[i][j] * 100) / 100);
}
});
